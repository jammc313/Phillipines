...
# 
# Here we are looking at admixed segment length distributions, D tests, and % shared archaic SNP blocks between Ayta and Papuan for Neanderthal and Denisovan. The model we are testing is the Null, with a single admixture event from Neanderthal into the shared ancestor of all Non-Africans, and a single Denisovan admixture event into the ancestor of Papuans and Ayta.
...
def set_up_demography(t67, t75, t54, t10, t20, t03, t83, t34, t49, ta1, ta2, ta3, f):
    ...
    admix = [msp.MassMigration(time=ta2, source=AYT, destination=DEN1, proportion=f[1]), #fraction from Denisovan 1 to Negrito/Papuan
...
def get_migrating_tracts(ts):
    migrating_tracts_DEN1, migrating_tracts_NEA = [], []
    # Get all tracts that migrated into archaic populations
    ...
    return np.array(migrating_tracts_DEN1), np.array(migrating_tracts_NEA) 
...
for aRun in range(numRuns):
    ...
    mig_DEN1, mig_NEA = get_migrating_tracts(sims)
    ...
    res_arr[aRun] = D_mean_arr[0][0], D_mean_arr[1][0], shared_DEN_PAPvsAYT, shared_NEA_PAPvsAYT, mean_tract_den1, mean_tract_nea


# In[ ]:


# function to return 2D array with dim0=(D_test_AYT, D_test_PAP, %shared_DEN, %_shared_NEA, mean_tractL_DEN1, mean_tractL_NEA) & dim1 = (mean, stdev, marginoferror)
...
np.savetxt('mean_res_NULL.txt', mean_res)
...
plt.savefig('plot_NULL_meanD.pdf', dpi=600, bbox_inches='tight')
...
plt.savefig('plot_NULL_shared.pdf', dpi=600, bbox_inches='tight')
...
plt.savefig('plot_NULL_meanL.pdf', dpi=600, bbox_inches='tight')
