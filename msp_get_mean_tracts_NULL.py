...
for aRun in range(numRuns):
    ...
    mean_tract_den_ayt = np.mean(mig_tracts_DEN1_AYT[:,1] - mig_tracts_DEN1_AYT[:,0], axis=0).astype(int)
    mean_tract_den_pap = np.mean(mig_tracts_DEN1_PAP[:,1] - mig_tracts_DEN1_PAP[:,0], axis=0).astype(int)
    ...
    res_arr[aRun] = D_mean_arr[0][0], D_mean_arr[1][0], shared_DEN_PAPvsAYT, shared_NEA_PAPvsAYT, mean_tract_den_ayt, mean_tract_den_pap, mean_tract_nea_ayt, mean_tract_nea_pap
